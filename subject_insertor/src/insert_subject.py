...
import codecs
from detokenizador import detok
...
def insert_first_person_subj(input_file, output_file):
    """
    ...
    for s in s_list:
        ...
        for vp in insertor.vp_list:
            ...
            try:
                ...
                pass
   # with codecs.open(output_file, 'w', encoding='utf-16') as fp:
   #     fp.write(insertor.dom.toxml())


	for t in s.getElementsByTagName('t'):
		ret += t.attributes['word'].value
		ret += ' '
    return ret
...
if __name__ == '__main__':
    ...
    assert len(argv) == 2, 'Usage: $python thisfile [input_file] [output_file]'
    #example:  python insert\ first\ person\ subj.py input_file.xml output_file.xml
    result = insert_first_person_subj(argv[1], argv[1])
    print detok(result).encode('utf-8')
